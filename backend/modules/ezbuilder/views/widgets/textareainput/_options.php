...
$rows = isset($options['rows'])?$options['rows']:$optionsTmp['rows'];
?>
...
<div class="well" style="padding: 15px; background-color: #dff0d8; border-color: #d6e9c6; ">
	...
	<div class="row">
	    <div class="col-md-3 ">
		<?= Html::label(Yii::t('ezform', 'Row'), 'options[rows]', ['class' => 'control-label']) ?>
		<?= Html::textInput('options[rows]', $rows, ['class' => 'form-control', 'type' => 'number']) ?>
	    </div>
