...
$dataItems = (isset($data['func']))?$data['func']:$data['items'];
//set input
$options['inline'] = $options['specific']['inline'];
unset($options['specific']);
	
$config = ArrayHelper::merge($options, ['name'=>$field['ezf_field_name'], 'value'=>$field['ezf_field_default'], 'data'=>$dataItems]);
...
$pathInput = [
    '{input}'=> \backend\modules\ezforms2\classes\EzformWidget::radioList($field['ezf_field_name'], $field['ezf_field_default'], $dataItems, $options),//Select2::widget($config),//
];
