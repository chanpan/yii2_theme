            ...
            if($this->value !== null){
                ...
                if($value!=''){
                    ...
                    if($event->name == ActiveRecord::EVENT_AFTER_DELETE){
                        @unlink(Yii::getAlias('@storage/ezform/audio/') . $fileName);
                    } else {
                        if (stristr($fileName, 'fileNameAuto') == TRUE) {
                            $nameEdit = true;
                            $newFileName = \common\lib\codeerror\helpers\GenMillisecTime::getMillisecTime() . '.mp3';
                            @copy(Yii::getAlias('@storage/ezform/audio/')."$foder/" . $fileName, Yii::getAlias('@storage/ezform/audio/') . $newFileName);
                            @unlink(Yii::getAlias('@storage/ezform/audio/') . "$foder/" . $fileName);
                        }
