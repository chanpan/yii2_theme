...
function renderTap(findTxt) {
    ...
    $('#conditionBox').html('<label><input type="checkbox" id="condition_auto" value="1" checked> Auto</label><ul id="conditionTabs" class="nav nav-tabs" role="tablist"></ul><div id="conditionTabContent" class="tab-content"></div>');
	...
	condArr[$(item).attr('id')] = {ezf_id:eid, ezf_field_name:ezf_field_name, ezf_field_value:ezf_field_value, cond_jump:'', cond_require:'', label_jump:'', label_require:'', var_jump:'', var_require:''};
	...
	if(newLoad){//edit field DB loading.
		...
		type   : 'POST',
		url    : conditionUrl,
		data   : {ezf_id:eid, ezf_field_name:ezf_field_name, ezf_field_value:ezf_field_value},
		dataType: 'JSON',
