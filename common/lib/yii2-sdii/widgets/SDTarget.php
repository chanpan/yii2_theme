...
class SDTarget extends Select2 {
    ...
    protected function renderInput()
    {
        ...
        echo '<div class="input-group" id="target-'.$this->ezf_field_id.'" >';
        echo $this->_loadIndicator . $this->embedAddon($input);
        echo '<span class="input-group-btn">';   
        echo Html::button('<i class="glyphicon glyphicon-eye-open"></i> ', ['data-toggle'=>'tooltip', 'title'=>'เปิดฟอร์มเป้าหมายที่เลือก', 'class'=>'btn btn-primary btn-open-ezform btn-edit', 'data-url'=>Url::to(['/ezforms2/ezform-data/'.$widgetName, 'ezf_id'=>$this->ezf_id, 'modal'=>"modal-{$this->ezf_field_id}", 'dataid'=>'']), 'data-id'=>$value, 'style'=>$value>0?'':'display: none;']).' ';
        if(!$disabled){
            echo Html::button('<i class="glyphicon glyphicon-plus"></i> ', ['data-toggle'=>'tooltip', 'title'=>'เพิ่มเป้าหมายใหม่', 'class'=>'btn btn-success btn-open-ezform btn-add', 'data-url'=>Url::to(['/ezforms2/ezform-data/ezform', 'ezf_id'=>$this->ezf_id, 'modal'=>"modal-{$this->ezf_field_id}" ])]).' ';
        }
        echo '</span>';
        echo '</div>';
        echo '</div> ';
    ...
    protected function registerJs()
    {
        ...
        if(hasForm){
            $('#target-{$this->ezf_field_id} .btn-open-ezform').hide();
        } else {
            $('#ezf-modal-box').append('$submodal');
        }
        
        $('#target-{$this->ezf_field_id} #$inputId').change(function(){
        ...
        $('#target-{$this->ezf_field_id} .btn-open-ezform').click(function(){
